...
@synthesize browserActionsToolBar = _browserActionsToolBar;
...
- (void)doubleTapOnBrowser
{
    ...
    NSUInteger browserFrameHeight;
    
    if (self.browserIsFullScreen)
    {
        if (IS_IPHONE_5) {
            browserFrameHeight = 417;
        } else {
            browserFrameHeight = 329;
        }
        ...
        [UIView animateWithDuration:0.5
                         ...
                         animations:^ { 
                             self.browser.frame = CGRectMake(0, 58, 320, browserFrameHeight);
                             self.browserView.alpha = 1.0f;
                             self.browserActionsToolBar.alpha = 1.0f;
                         } completion:^(BOOL finished) {
                             self.browserActionsToolBar.hidden = NO;
                         }];
    }
    else
    {
        if (IS_IPHONE_5) {
            browserFrameHeight = 519;
        } else {
            browserFrameHeight = 431;
        }
        ...
        [UIView animateWithDuration:0.5
                            ...
                            options:UIViewAnimationOptionCurveEaseInOut 
                         animations:^ { 
                             self.browser.frame = CGRectMake(0, 0, 320, browserFrameHeight);
                             self.browserView.alpha = 0.0f;
                             self.browserActionsToolBar.alpha = 0.0f;
                         } completion:^(BOOL finished) {
                             self.browserView.hidden = YES;
                             self.browserActionsToolBar.hidden = YES;
                         }];
    }
}
...
- (void)viewDidUnload
{
    ...
    [self setBrowserActionsToolBar:nil];
             ...
             navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if (mymeTypeWasChecked)
    {
        mymeTypeWasChecked = NO;
        return YES;
    }
...
//        {
//            downloadLink = [requestedUrl absoluteString];
...
//            
//            NSURLConnection *connection = [[NSURLConnection alloc] initWithRequest:newRequest delegate:self startImmediately:YES];
//            
//            if (!connection)
//            {
//                return YES;
...
//    }
//    
    if (contentType && [acceptedFileTypes rangeOfString:contentType].location != NSNotFound) 
    {
        NSLog(@"NNNNNNN");
        isFile = YES;
//        return;
...
//    [self.browser loadRequest:[connection originalRequest]];
...
//            mymeTypeWasChecked = YES;
//            
//            [self.browser loadRequest:[NSURLRequest requestWithURL:[response URL]]];
//        }
//        
//        [connection cancel];
//    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    {
        downloadLink = [defaults stringForKey:@"DownloadLink"];
...
//    BookmarksViewController_iPhone *bookmarksViewController_iPhone = [[BookmarksViewController_iPhone alloc] initWithNibName:@"BookmarksViewController_iPhone" bundle:nil];
//    
//    [self.navigationController presentModalViewController:bookmarksViewController_iPhone animated:YES];
    
    BookmarksViewController_iPhone *bookmarksViewController_iPhone = [[BookmarksViewController_iPhone alloc] initWithNibName:@"BookmarksViewController_iPhone" bundle:nil];
    UINavigationController *nc = [[UINavigationController alloc] initWithRootViewController:bookmarksViewController_iPhone];
